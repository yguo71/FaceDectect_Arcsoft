package com.example.facedectect_arcsoft.activity.faceserver;
...
import com.example.facedectect_arcsoft.activity.model.FaceRegisterInfo;
...
public class FaceServer {
    ...
    public boolean init(Context context) {
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                int engineCode = faceEngine.init(context, FaceEngine.ASF_DETECT_MODE_VIDEO, FaceEngine.ASF_OP_0_HIGHER_EXT, 16, 1, FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT);
    ...
    public List<String> getAllRegisteredName(){
        List<String> nameList = new ArrayList<>();
        for(FaceRegisterInfo faceRegisterInfo:faceRegisterInfoList){
            nameList.add(faceRegisterInfo.getName());
        }
        return nameList;
    }

    public int deleteRegisteredFaceInfo(List<String> toBeDeletedName, Context context){
        synchronized (this) {
            if (context == null) {
                return 0;
            }
            ...
            if (featureFileDir.exists() && featureFileDir.isDirectory()) {
                ...
                if (featureFiles != null && featureFiles.length > 0) {
                    for (File featureFile : featureFiles) {
                        if (toBeDeletedName.contains(featureFile.getName())) {
                            featureFile.delete();
                            deletedFeatureCount++;
                        }
                    }
            ...
            if (imgFileDir.exists() && imgFileDir.isDirectory()) {
                ...
                if (imgFiles != null && imgFiles.length > 0) {
                    for (File imgFile : imgFiles) {
                        if (toBeDeletedName.contains(imgFile.getName())) {
                            imgFile.delete();
                            deletedImageCount++;
                        }
                    }
            ...
            for(FaceRegisterInfo faceRegisterInfo:faceRegisterInfoList){
                if(toBeDeletedName.contains(faceRegisterInfo.getName())){
                        ...
                        if (bitmap != null) {
                            switch (faceInfoList.get(0).getOrient()) {
                                ...
                                case FaceEngine.ASF_OC_90:
                                    bitmap = toturn(bitmap, 90);
                                    needAdjust = true;
                                    break;
                                case FaceEngine.ASF_OC_180:
                                    bitmap = toturn(bitmap, 180);
                                    needAdjust = true;
                                    break;
                                case FaceEngine.ASF_OC_270:
                                    bitmap = toturn(bitmap, 270);
                                    needAdjust = true;
                                    break;
                                default:
                                    break;
